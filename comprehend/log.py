...
class ComprehendLog:
    ...
    def _output(self,level,msg,args):
        ...
        try:
            if len(args)>0:
                ...
                for arg in args:
                    if isinstance(arg,Exception):
                        t.append(traceback.format_exc(arg))
                    else:
                        t.append(arg) 
                args=tuple(t)
                msg=msg%args
            ...
            msg=msg+"\r\n"
            s="[%s] %04d-%02d-%02d %02d:%02d:%02d.%03d (0x%04X):%s"%(getLevelString(level),now.year,now.month,now.day,now.hour,now.minute,now.second,now.microsecond/1000,(threading.get_ident()>>8)&0xffff,msg)
            prefix=''
            if level==TRACE:
                if not isWindows:prefix='\033[0;37m'          
            elif level==DEBUG:
                if not isWindows:prefix='\033[0;34m'          
            elif level==INFORMATION:
                if not isWindows:prefix='\033[1;32m'          
            elif level==WARNING:
                if not isWindows:prefix='\033[1;33m'          
            elif level==ERROR:
                if not isWindows:prefix='\033[1;31m'          
            else:
                pass
            self._lock.acquire()
            try:
            ...
            print("ComprehendLog._output crash!err=%s"%traceback.format_exc(e))
...
class NamedLog:
    ...
    def info(self,msg,*args):
        msg=self.name+msg
        self.log.log(INFORMATION, msg,args)
    def i(self,msg,*args):
        msg=self.name+msg
        self.log.log(INFORMATION, msg,args)
    def warn(self,msg,*args):
        msg=self.name+msg
        self.log.log(WARNING, msg,args)
    def w(self,msg,*args):
        msg=self.name+msg
        self.log.log(WARNING, msg,args)
    def error(self,msg,*args):
